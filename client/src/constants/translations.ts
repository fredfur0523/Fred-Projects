...
export const TRANSLATIONS = {
	pt: {
		...
		noAnaplanData:
			"Nenhum dado Anaplan carregado. Para KPIs priorizados use 2025 acumulado: salve em client/public/anaplan-kpis-2025.json (ou anaplan-kpis.json). Use o MCP anaplan-supply-chain-kpi (aggregate_kpis).",
		yearLabel: "Ano",
		execObjective:
			"Verificar se o processo de maturidade tecnológica tem efeito e impacto nos resultados das operações.",
		...
		whatAreClustersBody:
			"Clusters são agrupamentos homogêneos de operações para comparação justa: mesmo porte (volume G1/G2/G3) e mesmo estágio de maturidade (L0 a L4). Assim evitamos comparar sites muito diferentes entre si e isolamos o efeito da maturidade sobre os indicadores.",
		...
		studyOseTtpSub:
			"Cruzamento a nível planta: maturidade (L0–L4) × OSE e TTP. Dados YTD. Alinhado à metodologia Supply/Packaging (escopo MTH/AC/BOPSLINE/LINE, exclusão keg).",
		studyOseTtpNoData:
			"Para OSE canônico: inclua PG-R0060 (EPT) e PG-K4039 (OST) em anaplan-ose-ttp-2025.json ou anaplan-kpis-2025.json. OSE = ΣEPT/ΣOST (%). TTP: PG-K0412. Fallback OSE: PG-K4038.",
		withKpi: "Com KPI",
		oseVolumeMethodology:
			"OSE canônica = ΣEPT/ΣOST (PG-R0060/PG-K4039), exibida em %. Fonte: tabela consolidada; escopo MTH/AC/BOPSLINE/LINE; exclusão keg. Dashboard exibe Anaplan PG-K4038 quando disponível. Detalhes: docs/METODOLOGIA_VOLUME_OSE.md",
		oseMethodologyOutput:
			"Análise em 3 blocos: (1) Resumo executivo global, (2) Visão por zona, (3) Deep dive exceções + NST + plano de ação.",
		oseMissingHint:
			"Para OSE: inclua PG-R0060 (EPT) e PG-K4039 (OST) em client/public/anaplan-ose-ttp-2025.json. Ou use PG-K4038 como fallback.",
		...
		quadrantInsight: (vpoDelta: string, techDelta: string) =>
			`VPO faz ${vpoDelta} p.p. de diferença · Tech faz ${techDelta} p.p.`,
		...
		statusCol: "Status",
		zoneCol: "Zona",
		topPerformersVpo: (top: string, bot: string, gap: string) =>
			`VPO top performers: ${top}% · base: ${bot}% · diferença: ${gap}pp`,
		noDomainData: "Dados insuficientes para este domínio.",
		migrationTitle: "Condicionantes de Migração de Nível",
		migrationSub:
			"O que deve estar em ordem antes de investir para subir de nível — por domínio e para o site completo",
		...
		portfolioSub:
			"Maturidade e cobertura dos produtos globais por domínio e zona — base para conversas de descomissionamento",
		...
		portfolioDataUnavailableTooltip: (zone: string, domain: string) =>
			`Os dados de cobertura de produto para ${zone}/${domain} não estão disponíveis nesta versão do relatório`,
		...
		portfolioSitesNeedL2: (n: number, domain: string) =>
			`${n} sites precisam atingir L2 em ${domain} para viabilizar decom`,
		...
		vpoCalloutBody: (below: number, total: number) =>
			`${below} de ${total} sites estão abaixo de VPO 80% (VPO Médio/Básico). Para esses sites, o retorno de investimento em maturidade digital é marginal sem primeiro elevar o VPO.`,
		...
		anaplanPlaceholderBody:
			"Carregue os dados Anaplan para ver a análise cruzada. Use anaplan-ose-ttp-2025.json em client/public/.",
		anaplanPlaceholderHowTo: "Como carregar?",
		anaplanPlaceholderSteps:
			"Salve o arquivo anaplan-ose-ttp-2025.json (ou anaplan-kpis-2025.json) na pasta client/public/ e recarregue a página.",
		...
		execMethodologyCallout:
			"O score L0–L4 representa o nível mínimo de maturidade entre todos os domínios ativos do site. O score N3/N4 é baseado em frações de capacidade ponderadas.",
		...
		methodologyWhyDifferBody:
			"O score N3/N4 é mais rigoroso — avalia se capacidades específicas estão funcionando, não apenas se o produto está implantado.",
		methodologyHelp: "?",
		...
		methSec1Body:
			'O score do site = mínimo dos scores de domínios ativos (excluindo Utilities). Um domínio é "ativo" quando tem pelo menos um produto Live. Domínios sem produto ativo não entram no cálculo. Utilities (UT) está excluído até que as definições de nível sejam estabelecidas.',
		methSec2Title: "Níveis de Maturidade (L0–L4)",
		methSec3Title: "Regras por Domínio",
		methSec4Title: "Regra de Produto Dominante",
		methSec4Body:
			"Quando um site tem múltiplos produtos ativos no mesmo domínio, o dominante é aquele que cobre o maior número de N4 capabilities no nível atual do site. Empate: Legacy prevalece sobre Global.",
		methSec5Title: "Sites Excluídos do Racional de Zona",
		methSec5Body:
			"Sites com nenhum produto ativo em nenhum domínio são mantidos na lista mas excluídos dos cálculos de funil por zona. Geralmente representam erros de cadastro ou duplicatas.",
	...
	en: {
		...
		noAnaplanData:
			"No Anaplan data loaded. For prioritized KPIs use 2025 accumulated: save to client/public/anaplan-kpis-2025.json (or anaplan-kpis.json). Use MCP anaplan-supply-chain-kpi (aggregate_kpis).",
		yearLabel: "Year",
		execObjective:
			"Verify whether the technology maturity process has a measurable effect and impact on operational results.",
		execQuestion: "Do sites with higher digital maturity deliver better operational results?",
		whatAreClusters: "What are clusters?",
		whatAreClustersBody:
			"Clusters are homogeneous groups of operations for fair comparison: same scale (volume G1/G2/G3) and same maturity stage (L0 to L4). This avoids comparing very different sites and isolates the effect of maturity on indicators.",
		...
		studyOseTtpSub:
			"Plant-level cross: maturity (L0–L4) × OSE and TTP. YTD data. Aligned to Supply/Packaging methodology (scope MTH/AC/BOPSLINE/LINE, keg excluded).",
		studyOseTtpNoData:
			"For canonical OSE include PG-R0060 (EPT) and PG-K4039 (OST) in anaplan-ose-ttp-2025.json or anaplan-kpis-2025.json. OSE = ΣEPT/ΣOST (%). TTP: PG-K0412. Fallback OSE: PG-K4038.",
		withKpi: "With KPI",
		oseVolumeMethodology:
			"Canonical OSE = ΣEPT/ΣOST (PG-R0060/PG-K4039), shown in %. Source: consolidated table; scope MTH/AC/BOPSLINE/LINE; keg excluded. Dashboard shows Anaplan PG-K4038 when available. Details: docs/METODOLOGIA_VOLUME_OSE.md",
		oseMethodologyOutput:
			"Analysis in 3 blocks: (1) Global executive summary, (2) View by zone, (3) Deep dive exceptions + NST + action plan.",
		oseMissingHint:
			"For OSE add PG-R0060 (EPT) and PG-K4039 (OST) to client/public/anaplan-ose-ttp-2025.json. Or use PG-K4038 as fallback.",
		noDataShort: "No data",
		// ── Stakeholder Executive View ──
		...
		quadrantInsight: (vpoDelta: string, techDelta: string) =>
			`VPO makes ${vpoDelta} p.p. difference · Tech makes ${techDelta} p.p.`,
		...
		zoneCol: "Zone",
		topPerformersVpo: (top: string, bot: string, gap: string) =>
			`VPO top performers: ${top}% · bottom: ${bot}% · gap: ${gap}pp`,
		...
		portfolioTitle: "Portfolio Intelligence: Global vs Legacy",
		portfolioSub:
			"Maturity and coverage of global products by domain and zone — foundation for decommissioning conversations",
		...
		portfolioDataUnavailableTooltip: (zone: string, domain: string) =>
			`Product coverage data for ${zone}/${domain} is not available in this report version`,
		...
		vpoCalloutBody: (below: number, total: number) =>
			`${below} of ${total} sites are below VPO 80% (Basic/Intermediate VPO). For these sites, digital maturity investment ROI is marginal without first raising VPO.`,
		...
		anaplanPlaceholderBody:
			"Load Anaplan data to see the cross analysis. Use anaplan-ose-ttp-2025.json in client/public/.",
		anaplanPlaceholderHowTo: "How to load?",
		anaplanPlaceholderSteps:
			"Save anaplan-ose-ttp-2025.json (or anaplan-kpis-2025.json) to the client/public/ folder and reload the page.",
		// ── Executive Summary (Sprint 4) ──
		...
		execMethodologyCallout:
			"The L0–L4 score represents the minimum maturity level across all active domains for each site. The N3/N4 score is based on weighted capability fractions.",
		...
		methodologyWhyDifferBody:
			"The N3/N4 score is more rigorous — it evaluates whether specific capabilities are functioning, not just whether the product is deployed.",
		methodologyHelp: "?",
		...
		methSec1Body:
			'The site score = minimum of active domain scores (excluding Utilities). A domain is "active" when it has at least one Live product. Domains without active products are excluded from the calculation. Utilities (UT) is excluded until level definitions are established.',
		methSec2Title: "Maturity Levels (L0–L4)",
		methSec3Title: "Rules by Domain",
		methSec4Title: "Dominant Product Rule",
		methSec4Body:
			"When a site has multiple active products in the same domain, the dominant one is the one covering the most N4 capabilities at the current site level. Tie-break: Legacy prevails over Global.",
		methSec5Title: "Sites Excluded from Zone Analysis",
		methSec5Body:
			"Sites with no active product in any domain are kept in the list but excluded from zone funnel calculations. These usually represent registration errors or duplicates.",
		methL0Name: "Non Digital",
		...
		methMaxNote:
			"Maximum score currently observed in the network = 2.6. L3/L4 are projections based on the methodology.",
		methDomGlobal: "Global",
