...
import type { ProductType, Site, SiteVolumeMix, VolumeComplexityCluster } from "../types";
...
export function getSiteComplexityBands(
	sites: Site[],
	volumeMix: Map<string, SiteVolumeMix>,
): Record<string, "L" | "M" | "H"> {
	const rawBySite: Record<string, number> = {};
	sites.forEach((s) => {
		rawBySite[s.name] = getSiteComplexity(s, volumeMix);
	});
...
export function getSiteClusterKey(
	site: Site,
	volumeMix: Map<string, SiteVolumeMix>,
...
export function buildVolumeComplexityClusters(
	...
	return Array.from(clusters.entries())
		.map(([clusterKey, clusterSites]) => {
			const [volumeBand, complexityBandVal] = clusterKey.split("_");
			const withKpi = clusterSites.filter(
				(s) => siteOseTtp[s.name] && (siteOseTtp[s.name].ose != null || siteOseTtp[s.name].ttp != null),
			);
			...
			return {
				clusterKey,
				volumeBand,
				complexityBand: complexityBandVal,
