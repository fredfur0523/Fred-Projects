...
export function computeStatsAnalysis(impactByLevel: ImpactByLevel[]): StatsAnalysis {
		...
		return {
			...
			narrative:
				n === 0
					...
					: "É necessário mais de um nível com dados para correlacionar maturidade e resultado.",
		};
	}
	...
	if (corr != null) {
		if (corr > 0.3)
			narrative =
				"Há associação positiva entre nível de maturidade tecnológica e resultado operacional: quanto maior o nível (L0→L4), maior tende a ser o indicador composto. O investimento em maturidade mostra efeito nos resultados.";
		else if (corr < -0.3)
			narrative = "Há associação negativa entre maturidade e indicador: revisar métricas ou amostra.";
		else
			narrative =
				"A correlação entre maturidade e resultado é fraca no período. Pode ser necessário mais tempo ou mais dados para observar o efeito.";
	}
	return {
		correlation: corr,
		byLevel: impactByLevel.map((i) => ({ level: i.level, mean: i.composite, count: i.withKpiCount })),
...
export function computeDomainOseCorrelation(
	...
	const GHQ_PCT: Record<string, number> = {
		Management: 63,
		Quality: 1,
		"Packaging Performance": 1,
		"Brewing Performance": 39,
		Safety: 45,
		Maintenance: 33,
		"Data Acquisition": 79,
		"MasterData Management": 93,
		Utilities: 4,
	};
	return DOMAIN_KEYS.filter((d) => d.key !== "Total Global")
		.map((dk) => {
			const pairs = sites
				.map((s) => ({ score: s.scores[dk.key] ?? 0, ose: siteOseTtp[s.name]?.ose ?? null }))
				.filter((p) => p.score > 0 && p.ose != null);
...
export function computePillarPartials(
	...
	return PILS.map((p) => {
		...
		const ps = rows.map((r) => r.pillar!);
		const os = rows.map((r) => r.ose!);
		...
		const cls =
			rPartial != null && Math.abs(rPartial) > 0.15
				? ("own" as const)
				: rPartial != null && Math.abs(rPartial) < 0.05
					? ("spurious" as const)
					: ("marginal" as const);
...
export function buildDomainAnalysis(
	...
	const rows: DomainAnalysisRow[] = domainKeys.map((d) => {
		...
		const avgOsePct = withOse.length
			? withOse.reduce((a, s) => a + siteOseTtp[s.name].ose! * 100, 0) / withOse.length
			: null;
	...
	const corrOse =
		n >= 2
			? pearson(
					withBoth.map((r) => r.avgMaturity),
					withBoth.map((r) => r.avgOsePct!),
				)
			: null;
	const corrTtp =
		n >= 2
	...
	if (corrOse != null || corrTtp != null) {
		if ((corrOse != null && corrOse > 0.3) || (corrTtp != null && corrTtp > 0.3))
			answerMatureDomain =
				"Sim. Domínios com maturidade média mais alta tendem a apresentar melhores OSE e/ou TTP (correlação positiva). Investir em maturidade por domínio está alinhado a melhores resultados.";
		else if ((corrOse != null && corrOse < -0.3) || (corrTtp != null && corrTtp < -0.3))
			answerMatureDomain =
				"Não no período. A correlação entre maturidade do domínio e indicadores é negativa; revisar métricas ou amostra.";
		else
			answerMatureDomain =
				"A correlação é fraca: ter um domínio mais maduro não mostra, nos dados atuais, relação forte com OSE/TTP. Pode ser necessário mais tempo ou mais dados.";
	}
	const highMaturityLowIndicators = rows.filter(
		(r) => r.avgMaturity >= 2 && ((r.avgOsePct != null && r.avgOsePct < 70) || (r.avgTtp != null && r.avgTtp < 2)),
	);
	let answerHighAvg =
		"Média de maturidade alta com indicadores ainda baixos pode indicar rollout recente (resultados ainda não refletidos) ou outros fatores (volume, mix, operação). Não é necessariamente mau sinal se a tendência for de melhoria.";
	if (highMaturityLowIndicators.length > 0)
		answerHighAvg =
			`${highMaturityLowIndicators.length} domínio(s) com maturidade média ≥2 e OSE <70% ou TTP <2. ` + answerHighAvg;
	...
	let answerPortfolio =
		"Compare a maturidade por domínio (tabela abaixo) com OSE e TTP. Em geral, priorizar domínios que impactam supply (DA, MDM, PP, BP) e elevar para L2+ tende a melhorar indicadores quando há correlação positiva.";
	if (bestOse || bestTtp)
		answerPortfolio =
			`Melhor OSE médio: ${bestOse?.domainShort ?? "—"}. Melhor TTP médio: ${bestTtp?.domainShort ?? "—"}. ` +
			answerPortfolio;
	return {
		rows,
	...
	return clusters
		.map((c) => {
			...
			const avgOsePct = withOse.length
				...
				: null;
			const avgTtp = withTtp.length ? withTtp.reduce((a, s) => a + siteOseTtp[s.name].ttp!, 0) / withTtp.length : null;
			const avgMaturity = clusterSites.length
				? clusterSites.reduce((a, s) => a + (s.scores["Total Global"] ?? 0), 0) / clusterSites.length
				: 0;
			return {
				clusterKey: c.key,
				label: c.label,
...
export function buildPageInsights(
	...
	if (hasOseTtpStudy && impactOseTtp.length) {
		const byLevel = impactOseTtp
			...
			.map(
				(i) =>
					`${i.level}: OSE ${i.avgOse != null ? (i.avgOse * 100).toFixed(1) + "%" : "—"}, TTP ${i.avgTtp != null ? i.avgTtp.toFixed(2) : "—"} (${i.withKpiCount} sites)`,
			);
	...
	if (volumeComplexityClusters.length) {
		...
		if (bestCluster)
			parts.push(
				`Entre os clusters (volume × complexidade), melhor OSE médio em ${bestCluster.clusterKey} (${bestCluster.avgOsePct?.toFixed(1)}%).`,
			);
		if (worstTtp && volumeComplexityClusters.length > 1)
			parts.push(
				`Maior TTP médio no cluster ${worstTtp.clusterKey} (${worstTtp.avgTtp?.toFixed(2)}), sugerindo maior complexidade operacional.`,
			);
	}
	...
	const bestDomainOse = domainAnalysis.rows
		.filter((r) => r.avgOsePct != null)
		.sort((a, b) => (b.avgOsePct ?? 0) - (a.avgOsePct ?? 0))[0];
	const bestDomainTtp = domainAnalysis.rows
		.filter((r) => r.avgTtp != null)
		.sort((a, b) => (b.avgTtp ?? 0) - (a.avgTtp ?? 0))[0];
	if (bestDomainOse || bestDomainTtp)
		parts.push(
			`Por domínio, destaque em OSE: ${bestDomainOse?.domainShort ?? "—"}; em TTP: ${bestDomainTtp?.domainShort ?? "—"}. Priorizar maturidade em DA, MDM, PP e BP tende a reforçar resultados quando a correlação for positiva.`,
		);
