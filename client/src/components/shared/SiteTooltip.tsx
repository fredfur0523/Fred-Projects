		...
		<>
			...
			{visible &&
				ReactDOM.createPortal(
					...
					>
						<div
							className={"flex items-center gap-2 border-b pb-2 mb-3 " + (dark ? "border-gray-700" : "border-gray-800")}
						>
							...
							<div className="space-y-1">
								{pageSites.map((s, i) => {
									...
									return (
										<div
											key={i}
											className="flex items-center justify-between gap-2 py-1 border-b border-gray-800 last:border-0"
										>
											<div className="flex items-center gap-2 min-w-0">
												<span
													className={
														"px-1.5 py-0.5 rounded-full text-[9px] font-black border " +
														(dark ? zc.darkBg + " " + zc.darkText : "bg-gray-800 text-gray-200") +
														" border-transparent"
													}
												>
													{s.zone}
											...
											<span className="text-gray-400 text-[10px] whitespace-nowrap flex-shrink-0">
												{(s.volume / 1e6).toFixed(1)}M · {s.group}
											</span>
										</div>
									);
								})}
						...
						{totalPages > 1 && (
							<div className="flex items-center justify-between mt-3 pt-2 border-t border-gray-800">
								...
								<span className="text-[10px] text-gray-500">
									{page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, sites.length)} de {sites.length}
								</span>
								<button
