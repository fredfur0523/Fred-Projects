...
export const FloatingComparisonBar: React.FC<{
	...
	return ReactDOM.createPortal(
		<div className="fixed bottom-0 left-0 right-0 z-[90] flex justify-center p-3 pointer-events-none">
			<div
				className={
					"pointer-events-auto flex items-center gap-3 px-5 py-3 rounded-2xl border shadow-2xl backdrop-blur-lg " + bar
				}
				style={{ maxWidth: 600 }}
			>
				...
				{selected.map((name) => (
					...
					>
						{name}
						<button onClick={() => onRemove(name)} className="ml-0.5 hover:text-red-400 transition-colors">
							×
						</button>
					</span>
				))}
