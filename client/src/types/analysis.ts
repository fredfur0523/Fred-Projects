...
export interface DomainOseRow {
	domain: string;
	short: string;
	ghqPct: number;
	rOse: number | null;
	n: number;
	cls: "global" | "mixed" | "legacy";
}

export interface PillarPartialRow {
	pillar: string;
	rSimple: number | null;
	rPartial: number | null;
	n: number;
	cls: "own" | "spurious" | "marginal";
}
